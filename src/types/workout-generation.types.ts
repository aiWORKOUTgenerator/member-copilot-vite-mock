 ...
 * These should be removed once all services are updated
 */

// Legacy app-level request (for migration)
export interface LegacyAppWorkoutRequest {
  type: 'quick' | 'detailed';
  profileData: ProfileData;
  waiverData?: LiabilityWaiverData;
  workoutFocusData: PerWorkoutOptions;
...
// Legacy AI service request (for migration)
export interface LegacyAIServiceWorkoutRequest {
  userProfile: UserProfile;
  workoutOptions: PerWorkoutOptions;
  preferences: WorkoutPreferences;
  constraints?: WorkoutConstraints;
 ...
 */
export class LegacyRequestConverter {
  
  /**
   * Convert legacy app request to unified format
   */
  static fromLegacyApp(request: LegacyAppWorkoutRequest): WorkoutGenerationRequest {
    return {
  ...
  /**
   * Convert legacy AI service request to unified format
   */
  static fromLegacyAIService(request: LegacyAIServiceWorkoutRequest): WorkoutGenerationRequest {
    return {
      workoutType: 'detailed', // Default for AI service requests
      profileData: {} as ProfileData, // Will need to be populated
      workoutFocusData: request.workoutOptions,
      userProfile: request.userProfile,
      preferences: request.preferences,
      constraints: request.constraints,
      environmentalFactors: request.environmentalFactors
  ...
  /**
   * Convert unified request to legacy app format
   */
  static toLegacyApp(request: WorkoutGenerationRequest): LegacyAppWorkoutRequest {
    return {
  ...
  /**
   * Convert unified request to legacy AI service format
   */
  static toLegacyAIService(request: WorkoutGenerationRequest): LegacyAIServiceWorkoutRequest {
    const aiData = WorkoutRequestAdapter.extractAIServiceData(request);
    return {
      userProfile: aiData.userProfile,
      workoutOptions: aiData.workoutOptions,
      preferences: aiData.preferences || WorkoutRequestAdapter.createDefaultEnhancements(
        request.userProfile, 
        request.workoutFocusData
      ).preferences,
      constraints: aiData.constraints,
      environmentalFactors: aiData.environmentalFactors
    };
  }
}
