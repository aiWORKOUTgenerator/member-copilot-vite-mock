...
export const validateEquipment = (value: string[] | EquipmentSelectionData, allOptions?: any): ValidationResult => {
  ...
  if (allOptions?.customization_focus) {
    ...
    if (focus === 'strength' && equipment.includes('Bodyweight Only') && equipment.length === 1) {
      result.warnings = ['Bodyweight-only training may limit strength development'];
      result.recommendations = ['Consider adding weights or resistance equipment'];
    }
    
    if (focus === 'power' && !equipment.some(eq => ['Barbell', 'Dumbbells', 'Kettlebells'].includes(eq))) {
  ...
  if (typeof value === 'object' && !Array.isArray(value) && value?.location) {
    ...
    if (location === 'home' && equipment.includes('Barbell')) {
    ...
    if (location === 'outdoor' && equipment.includes('Machines')) {
      result.isValid = false;
